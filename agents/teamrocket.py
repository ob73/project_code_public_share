...
class Agent(object):
    def __init__(self, agent_number, params={}):
        ...
        self.current_streak = 0 # keep track of win/lose streaks to increase/reduce prices exponentially
    ...
    def _process_last_sale(self, last_sale, profit_each_team):
        ...
        # losing case, reduce prices/alpha
        if did_customer_buy_from_opponent:
            if self.latest_winner == 0: 
                self.current_streak += 1
            else:
                self.current_streak = 1
            self.latest_winner = 0
            price_diff = my_last_prices[which_item_customer_bought] - opponent_last_prices[which_item_customer_bought]
            price_diff_percentage = price_diff / my_last_prices[which_item_customer_bought] # how much % i should reduce to match
            self.alpha = self.alpha * ( (1- (price_diff_percentage + 0.01)) ** self.current_streak) # reduce prices/alpha by (diff + 1)%, +1 to cut more aggresively to secure sale
